...
export default function ProfileScreen() {
  ...
  const calculateBMI = () => {
    ...
    let statusData = { text: "Норма", color: "#00b894", advice: "Отличный баланс! Продолжайте в том же духе." };
    if (val < 18.5) statusData = { text: "Дефицит", color: "#fab1a0", advice: "Вам стоит увеличить калорийность рациона." };
    else if (val >= 25 && val < 30) statusData = { text: "Избыток", color: "#fdcb6e", advice: "Рекомендуется снизить сахар и добавить ходьбу." };
    else if (val >= 30) statusData = { text: "Ожирение", color: "#e17055", advice: "Необходим дефицит калорий и контроль врача." };

    setResult({ bmi: bmiVal, status: statusData.text, color: statusData.color, advice: statusData.advice });
    ...
    <ScrollView style={styles.container}>
      ...
      {result && (
        <View style={[styles.resultCard, { borderTopColor: result.color }]}>
          ...
          <View style={[styles.statusTag, { backgroundColor: result.color }]}>
            <Text style={styles.statusTagText}>{result.status}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  statusTag: { alignSelf: 'center', paddingHorizontal: 20, paddingVertical: 5, borderRadius: 10, marginTop: 10 },
  statusTagText: { color: 'white', fontWeight: 'bold' },
